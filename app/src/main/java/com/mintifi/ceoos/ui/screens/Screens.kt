...
fun DashboardScreen(viewModel: MainViewModel, onNavigateToRecord: () -> Unit, onNavigateToCopilot: () -> Unit, onNavigateToTodo: () -> Unit, onNavigateToReminders: () -> Unit) {
    ...
    val playingId       by viewModel.playingSessionId.collectAsState()
    ...
    taskToDelete?.let { task ->
        AlertDialog(onDismissRequest={taskToDelete=null}, title={Text("Delete Task?")}, text={Text("\"" + task.title + "\" will be permanently deleted.")},
    ...
    Column(Modifier.fillMaxSize().background(MintifiColors.Background).verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement=Arrangement.spacedBy(16.dp)) {
        ...
        Row(verticalAlignment=Alignment.CenterVertically) {
            ...
            Text("Good ${when{h<12->"morning";h<17->"afternoon";else->"evening"}}, Pradeep", style=MaterialTheme.typography.bodyMedium, color=MintifiColors.TextSecondary)
        }
        ...
        if (!isOnline || offlineCount > 0) {
            Card(colors=CardDefaults.cardColors(containerColor=if(!isOnline)MintifiColors.DangerRed.copy(alpha=0.1f) else MintifiColors.WarnAmber.copy(alpha=0.1f)), border=BorderStroke(0.5.dp,if(!isOnline)MintifiColors.DangerRed.copy(alpha=0.5f) else MintifiColors.WarnAmber.copy(alpha=0.5f)), shape=RoundedCornerShape(10.dp)) {
                Row(Modifier.padding(12.dp), horizontalArrangement=Arrangement.spacedBy(10.dp), verticalAlignment=Alignment.CenterVertically) {
                    ...
                    Text(if(!isOnline)"No internet — recordings will be queued" else "$offlineCount recording(s) queued", style=MaterialTheme.typography.bodySmall, color=if(!isOnline)MintifiColors.DangerRed else MintifiColors.WarnAmber, modifier=Modifier.weight(1f))
                    if(offlineCount>0&&isOnline) TextButton(onClick={viewModel.processOfflineQueue()}, colors=ButtonDefaults.textButtonColors(contentColor=MintifiColors.WarnAmber)){Text("Process")}
                }
            }
        }
        ...
        Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){DActionBtn("Record CEO Call",MintifiColors.Accent,Color.White,Modifier.weight(1f),onNavigateToRecord);DActionBtn("AI Copilot",MintifiColors.Surface,MintifiColors.Accent,Modifier.weight(1f),onNavigateToCopilot)}
        Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){DActionBtn("Today Tasks",MintifiColors.Surface,MintifiColors.InfoBlue,Modifier.weight(1f),onNavigateToTodo);DActionBtn("Reminders ($activeReminders)",MintifiColors.Surface,MintifiColors.AccentBlue,Modifier.weight(1f),onNavigateToReminders)}
            ...
            critical.forEach{task->
                Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.Surface),shape=RoundedCornerShape(10.dp),border=BorderStroke(0.5.dp,MintifiColors.Border),modifier=Modifier.fillMaxWidth()){
                    Row(Modifier.padding(12.dp),horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.CenterVertically){
                        ...
                        IconButton(onClick={taskToDelete=task},modifier=Modifier.size(28.dp)){Icon(Icons.Default.Delete,"Delete",tint=MintifiColors.DangerRed.copy(alpha=0.6f),modifier=Modifier.size(16.dp))}
                    }
                }
            }
        }
                ...
                Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.Surface),shape=RoundedCornerShape(10.dp),border=BorderStroke(0.5.dp,MintifiColors.Border),modifier=Modifier.fillMaxWidth()){
                    Column(Modifier.padding(12.dp),verticalArrangement=Arrangement.spacedBy(6.dp)){
                        Row(horizontalArrangement=Arrangement.spacedBy(10.dp),verticalAlignment=Alignment.CenterVertically){
                            ...
                            // Play audio button
                            if(s.audioPath.isNotBlank()){
                                val isThisPlaying=isPlayingAudio&&playingId==s.id
                                IconButton(onClick={if(isThisPlaying)viewModel.stopAudio() else viewModel.playSessionAudio(s)},modifier=Modifier.size(32.dp)){
                                    Icon(if(isThisPlaying)Icons.Default.Stop else Icons.Default.PlayCircle,"Play",tint=MintifiColors.Accent,modifier=Modifier.size(20.dp))
                                }
                            }
                            // Regenerate tasks
                            IconButton(onClick={viewModel.regenerateTasksForSession(s)},modifier=Modifier.size(32.dp)){Icon(Icons.Default.Refresh,"Regenerate",tint=MintifiColors.TextSecondary,modifier=Modifier.size(18.dp))}
                        ...
                        if(s.summary.isNotBlank()){Text(s.summary.take(100)+"...",style=MaterialTheme.typography.bodySmall,color=MintifiColors.TextSecondary,maxLines=2,overflow=TextOverflow.Ellipsis)}
                    }
                }
            }
        }
...
fun RecordScreen(viewModel: MainViewModel) {
    ...
    val picker = rememberLauncherForActivityResult(ActivityResultContracts.GetContent()) { uri: Uri? ->
        uri?.let { context.contentResolver.openInputStream(it)?.use { inp ->
            val tmp = File(context.cacheDir,"up_"+System.currentTimeMillis()+".m4a")
            tmp.outputStream().use{out->inp.copyTo(out)}
            viewModel.processAudioFile(tmp, viewModel.buildContextFromSpeaker(contextNote))
        }}
    }
    ...
    Column(Modifier.fillMaxSize().background(MintifiColors.Background).verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement=Arrangement.spacedBy(16.dp)) {
        ...
        Row(verticalAlignment=Alignment.CenterVertically) {
            ...
            Column{Text("CEO Call Capture",style=MaterialTheme.typography.headlineSmall,color=MintifiColors.TextPrimary);Text("Hindi + English  •  Offline queue enabled",style=MaterialTheme.typography.bodySmall,color=MintifiColors.TextSecondary)}
        }

        // ── SPEAKER QUICK SELECT ──────────────────────────────────────────────
        Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.Surface), shape=RoundedCornerShape(12.dp), border=BorderStroke(0.5.dp,MintifiColors.Border)) {
            Column(Modifier.padding(14.dp), verticalArrangement=Arrangement.spacedBy(10.dp)) {
                Text("Who is this discussion with?", style=MaterialTheme.typography.titleSmall, color=MintifiColors.TextPrimary)
                Row(horizontalArrangement=Arrangement.spacedBy(8.dp)) {
                    // CEO button — prominent
                    val ceoSelected = selectedSpk == "Anup Sir (CEO)"
                    Button(
                        onClick = { viewModel.selectSpeaker(if(ceoSelected) null else "Anup Sir (CEO)") },
                        colors = ButtonDefaults.buttonColors(
                            containerColor = if(ceoSelected) MintifiColors.Accent else MintifiColors.Surface2,
                            contentColor   = if(ceoSelected) Color.White else MintifiColors.TextPrimary
                        ),
                        border = if(ceoSelected) null else BorderStroke(1.dp, MintifiColors.Border),
                        shape  = RoundedCornerShape(10.dp),
                        elevation = ButtonDefaults.buttonElevation(0.dp)
                    ) {
                        Text("👑 CEO — Anup Sir", style=MaterialTheme.typography.titleSmall)
                        if(ceoSelected){ Spacer(Modifier.width(4.dp)); Icon(Icons.Default.CheckCircle,null,modifier=Modifier.size(16.dp)) }
                    }
                    // Other button
                    val otherSelected = selectedSpk != null && selectedSpk != "Anup Sir (CEO)"
                    OutlinedButton(
                        onClick = { viewModel.selectSpeaker(if(otherSelected) null else "Team Member") },
                        border = BorderStroke(1.dp, if(otherSelected) MintifiColors.AccentBlue else MintifiColors.Border),
                        shape  = RoundedCornerShape(10.dp),
                        colors = ButtonDefaults.outlinedButtonColors(contentColor = if(otherSelected) MintifiColors.AccentBlue else MintifiColors.TextSecondary)
                    ) { Text("👤 Other") }
                }
                if(selectedSpk!=null){
                    Row(verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(6.dp)){
                        Icon(Icons.Default.Info,null,tint=MintifiColors.Accent,modifier=Modifier.size(14.dp))
                        Text(if(selectedSpk=="Anup Sir (CEO)")"CEO context — P0/P1 priority weighted higher by AI" else "Team discussion context set",style=MaterialTheme.typography.bodySmall,color=MintifiColors.Accent)
                    }
                }
            }
        }
        ...
        if(!isOnline){Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.WarnAmber.copy(alpha=0.1f)),border=BorderStroke(0.5.dp,MintifiColors.WarnAmber.copy(alpha=0.5f)),shape=RoundedCornerShape(8.dp)){Row(Modifier.padding(10.dp),horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.CenterVertically){Icon(Icons.Default.WifiOff,null,tint=MintifiColors.WarnAmber,modifier=Modifier.size(16.dp));Text("Offline — recording will be queued",style=MaterialTheme.typography.bodySmall,color=MintifiColors.WarnAmber)}}}
        if(offlineCount>0&&isOnline){Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.SuccessGreen.copy(alpha=0.1f)),border=BorderStroke(0.5.dp,MintifiColors.SuccessGreen.copy(alpha=0.5f)),shape=RoundedCornerShape(8.dp)){Row(Modifier.padding(10.dp),horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.CenterVertically){Icon(Icons.Default.CloudUpload,null,tint=MintifiColors.SuccessGreen,modifier=Modifier.size(16.dp));Text("$offlineCount queued recording(s) ready",style=MaterialTheme.typography.bodySmall,color=MintifiColors.SuccessGreen,modifier=Modifier.weight(1f));TextButton(onClick={viewModel.processOfflineQueue()},colors=ButtonDefaults.textButtonColors(contentColor=MintifiColors.SuccessGreen)){Text("Process Now")}}}}
        ...
        uiMessage?.let{msg->
            Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.Accent.copy(alpha=0.1f)),border=BorderStroke(0.5.dp,MintifiColors.Accent.copy(alpha=0.4f)),shape=RoundedCornerShape(8.dp)){
                Row(Modifier.padding(12.dp),horizontalArrangement=Arrangement.spacedBy(10.dp),verticalAlignment=Alignment.CenterVertically){
                    if(isAnalyzing)CircularProgressIndicator(Modifier.size(14.dp),color=MintifiColors.Accent,strokeWidth=2.dp) else Icon(Icons.Default.CheckCircle,null,tint=MintifiColors.Accent,modifier=Modifier.size(16.dp))
        ...
        Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.Surface),shape=RoundedCornerShape(12.dp),border=BorderStroke(0.5.dp,if(isRecording)MintifiColors.Accent.copy(alpha=0.4f) else MintifiColors.Border)){
            Column(Modifier.padding(16.dp),horizontalAlignment=Alignment.CenterHorizontally){
                Row(horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.CenterVertically){
                    ...
                    if(selectedSpk!=null&&isRecording){Spacer(Modifier.width(4.dp));Text("• "+selectedSpk!!,style=MaterialTheme.typography.labelSmall,color=MintifiColors.Accent,fontFamily=FontFamily.Monospace,maxLines=1,overflow=TextOverflow.Ellipsis,modifier=Modifier.weight(1f))} else {Spacer(Modifier.weight(1f))}
                ...
                if(isRecording||amplitude>0f) WaveformVisualizer(amplitude=amplitude,isRecording=isRecording&&!isPaused)
                else Box(Modifier.fillMaxWidth().height(64.dp),contentAlignment=Alignment.Center){Text("Tap mic to start recording",style=MaterialTheme.typography.bodySmall,color=MintifiColors.TextMuted)}
                ...
                Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(16.dp,Alignment.CenterHorizontally),verticalAlignment=Alignment.CenterVertically){
                    when{
                        ...
                        isPaused->{
                            ...
                            Button(onClick={viewModel.stopAndProcess(viewModel.buildContextFromSpeaker(contextNote))},shape=CircleShape,modifier=Modifier.size(80.dp),colors=ButtonDefaults.buttonColors(containerColor=MintifiColors.DangerRed)){Icon(Icons.Default.Stop,null,tint=Color.White,modifier=Modifier.size(32.dp))}
                        }
                        else->{
        ...
        AnimatedVisibility(showText){
            Column(verticalArrangement=Arrangement.spacedBy(10.dp)){
                ...
                Button(onClick={if(manualText.isNotBlank()){viewModel.analyzeTextOnly(manualText,viewModel.buildContextFromSpeaker(contextNote));manualText="";showText=false}},enabled=manualText.isNotBlank()&&!isAnalyzing,colors=ButtonDefaults.buttonColors(containerColor=MintifiColors.Accent),shape=RoundedCornerShape(8.dp)){Text("Analyze Text",color=Color.White,fontWeight=FontWeight.SemiBold)}
            }
        }
    }
}
...
fun TaskLogScreen(viewModel: MainViewModel) {
    ...
    Column(Modifier.fillMaxSize()){
        Column(Modifier.background(MintifiColors.Surface).padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
            Row(verticalAlignment=Alignment.CenterVertically){
                Text("Master Ask Log",style=MaterialTheme.typography.headlineSmall,color=MintifiColors.TextPrimary); Spacer(Modifier.weight(1f))
                if(filter.priorities.isNotEmpty()||filter.statuses.isNotEmpty()){TextButton(onClick={viewModel.clearFilters()},colors=ButtonDefaults.textButtonColors(contentColor=MintifiColors.DangerRed)){Text("Clear Filters")}}
            }
            ...
            // PRIORITY FILTER — clear selected state with solid background + border
            ...
            Row(Modifier.horizontalScroll(rememberScrollState()),horizontalArrangement=Arrangement.spacedBy(8.dp)){
                Priority.values().forEach{p->
                    ...
                    val (selBg,selFg,selBorder)=when(p){
                        Priority.P0->Triple(MintifiColors.DangerRed,Color.White,MintifiColors.DangerRed)
                        Priority.P1->Triple(MintifiColors.WarnAmber,Color.White,MintifiColors.WarnAmber)
                        Priority.P2->Triple(MintifiColors.AccentBlue,Color.White,MintifiColors.AccentBlue)
                        Priority.P3->Triple(MintifiColors.SuccessGreen,Color.White,MintifiColors.SuccessGreen)
                    }
                    Box(
                        Modifier
                            .clip(RoundedCornerShape(8.dp))
                            .background(if(sel)selBg else MintifiColors.Surface2)
                            .border(1.5.dp,if(sel)selBorder else MintifiColors.Border,RoundedCornerShape(8.dp))
                            .clickable{viewModel.togglePriority(p)}
                            .padding(horizontal=14.dp,vertical=8.dp),
                        contentAlignment=Alignment.Center
                    ){
                        Row(horizontalArrangement=Arrangement.spacedBy(4.dp),verticalAlignment=Alignment.CenterVertically){
                            if(sel)Icon(Icons.Default.Check,null,tint=Color.White,modifier=Modifier.size(12.dp))
                            Text(p.label,style=MaterialTheme.typography.labelMedium,color=if(sel)Color.White else MintifiColors.TextSecondary,fontWeight=if(sel)FontWeight.Bold else FontWeight.Normal)
                        }
                    }
                }
            }

            // STATUS FILTER — same solid style
            ...
            Row(Modifier.horizontalScroll(rememberScrollState()),horizontalArrangement=Arrangement.spacedBy(8.dp)){
                listOf(TaskStatus.PENDING,TaskStatus.IN_PROGRESS,TaskStatus.COMPLETED,TaskStatus.WAITING,TaskStatus.ESCALATED).forEach{s->
                    ...
                    Box(
                        Modifier
                            .clip(RoundedCornerShape(8.dp))
                            .background(if(sel)MintifiColors.Accent else MintifiColors.Surface2)
                            .border(1.5.dp,if(sel)MintifiColors.Accent else MintifiColors.Border,RoundedCornerShape(8.dp))
                            .clickable{viewModel.toggleStatus(s)}
                            .padding(horizontal=14.dp,vertical=8.dp),
                        contentAlignment=Alignment.Center
                    ){
                        Row(horizontalArrangement=Arrangement.spacedBy(4.dp),verticalAlignment=Alignment.CenterVertically){
                            if(sel)Icon(Icons.Default.Check,null,tint=Color.White,modifier=Modifier.size(12.dp))
                            Text(s.label,style=MaterialTheme.typography.labelMedium,color=if(sel)Color.White else MintifiColors.TextSecondary,fontWeight=if(sel)FontWeight.Bold else FontWeight.Normal)
                        }
                    }
                }
            }
            ...
            // Message banner
            uiMessage?.let{msg->
                Card(colors=CardDefaults.cardColors(containerColor=MintifiColors.Accent.copy(alpha=0.1f)),border=BorderStroke(0.5.dp,MintifiColors.Accent.copy(alpha=0.3f)),shape=RoundedCornerShape(8.dp)){
                    Text(msg,style=MaterialTheme.typography.bodySmall,color=MintifiColors.Accent,modifier=Modifier.padding(10.dp))
                }
            }
        }
        Divider(color=MintifiColors.Border,thickness=0.5.dp)
        ...
        if(tasks.isEmpty()){
            ...
            // Group by projectGroup globally — so same project across sessions merge
            ...
            LazyColumn(contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
                grouped.forEach{(project,projectTasks)->
                    ...
                    items(projectTasks,key={it.id}){task->
                        TaskCardWithActions(task=task,onStatusChange={viewModel.updateTaskStatus(task.id,it)},onEdit={taskToEdit=task},onDelete={taskToDelete=task},onAddTodo={viewModel.addTaskToTodo(task)},onAddReminder={viewModel.addReminderFromTask(task)},fmt=fmt)
                    }
                }
    ...
    taskToEdit?.let{task->EditTaskSheet(task=task,onSave={updated->viewModel.updateTask(updated);taskToEdit=null},onDismiss={taskToEdit=null})}
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun TaskCardWithActions(task: Task, onStatusChange: (TaskStatus)->Unit, onEdit: ()->Unit, onDelete: ()->Unit, onAddTodo: ()->Unit, onAddReminder: ()->Unit, fmt: SimpleDateFormat) {
            ...
            if(expanded){
                ...
                Row(Modifier.horizontalScroll(rememberScrollState()),horizontalArrangement=Arrangement.spacedBy(6.dp)){
                    TaskStatus.values().forEach{s->
                        val sel=s==task.status
                        Box(Modifier.clip(RoundedCornerShape(6.dp)).background(if(sel)MintifiColors.Accent else MintifiColors.Surface2).border(1.dp,if(sel)MintifiColors.Accent else MintifiColors.Border,RoundedCornerShape(6.dp)).clickable{onStatusChange(s)}.padding(horizontal=10.dp,vertical=6.dp)){
                            Text(s.label,style=MaterialTheme.typography.labelSmall,color=if(sel)Color.White else MintifiColors.TextSecondary,fontWeight=if(sel)FontWeight.Bold else FontWeight.Normal)
                        }
                    }
                }
                // Action buttons row
                ...
                Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){
                    OutlinedButton(onClick=onAddTodo,border=BorderStroke(1.dp,MintifiColors.AccentBlue),shape=RoundedCornerShape(8.dp),colors=ButtonDefaults.outlinedButtonColors(contentColor=MintifiColors.AccentBlue),modifier=Modifier.weight(1f)){
                        Icon(Icons.Default.CheckBox,null,modifier=Modifier.size(14.dp));Spacer(Modifier.width(4.dp));Text("Add to To-Do",style=MaterialTheme.typography.labelSmall)
                    }
                    OutlinedButton(onClick=onAddReminder,border=BorderStroke(1.dp,MintifiColors.WarnAmber),shape=RoundedCornerShape(8.dp),colors=ButtonDefaults.outlinedButtonColors(contentColor=MintifiColors.WarnAmber),modifier=Modifier.weight(1f)){
                        Icon(Icons.Default.Alarm,null,modifier=Modifier.size(14.dp));Spacer(Modifier.width(4.dp));Text("Set Reminder",style=MaterialTheme.typography.labelSmall)
                    }
                }
            }
        }
...
private fun EditTaskSheet(task: Task, onSave: (Task)->Unit, onDismiss: ()->Unit) {
    ...
    ModalBottomSheet(onDismissRequest=onDismiss,containerColor=MintifiColors.Surface,dragHandle={Box(Modifier.padding(8.dp).width(40.dp).height(4.dp).background(MintifiColors.Border,RoundedCornerShape(2.dp)))}){
        Column(Modifier.fillMaxWidth().padding(horizontal=20.dp).padding(bottom=32.dp).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(14.dp)){
            ...
            Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){Priority.values().forEach{p->val sel=priority==p;Box(Modifier.clip(RoundedCornerShape(8.dp)).background(if(sel)MintifiColors.Accent else MintifiColors.Surface2).border(1.5.dp,if(sel)MintifiColors.Accent else MintifiColors.Border,RoundedCornerShape(8.dp)).clickable{priority=p}.padding(horizontal=14.dp,vertical=8.dp)){Text(p.label,style=MaterialTheme.typography.labelMedium,color=if(sel)Color.White else MintifiColors.TextSecondary,fontWeight=if(sel)FontWeight.Bold else FontWeight.Normal)}}}
            Text("Status:",style=MaterialTheme.typography.labelMedium,color=MintifiColors.TextSecondary)
            Row(Modifier.horizontalScroll(rememberScrollState()),horizontalArrangement=Arrangement.spacedBy(8.dp)){TaskStatus.values().forEach{s->val sel=status==s;Box(Modifier.clip(RoundedCornerShape(8.dp)).background(if(sel)MintifiColors.Accent else MintifiColors.Surface2).border(1.5.dp,if(sel)MintifiColors.Accent else MintifiColors.Border,RoundedCornerShape(8.dp)).clickable{status=s}.padding(horizontal=10.dp,vertical=8.dp)){Text(s.label,style=MaterialTheme.typography.labelSmall,color=if(sel)Color.White else MintifiColors.TextSecondary,fontWeight=if(sel)FontWeight.Bold else FontWeight.Normal)}}}
            Text("Category:",style=MaterialTheme.typography.labelMedium,color=MintifiColors.TextSecondary)
            LazyRow(horizontalArrangement=Arrangement.spacedBy(8.dp)){items(Category.values()){c->val sel=category==c;Box(Modifier.clip(RoundedCornerShape(8.dp)).background(if(sel)MintifiColors.Accent else MintifiColors.Surface2).border(1.5.dp,if(sel)MintifiColors.Accent else MintifiColors.Border,RoundedCornerShape(8.dp)).clickable{category=c}.padding(horizontal=10.dp,vertical=8.dp)){Text(c.emoji+" "+c.label,style=MaterialTheme.typography.labelSmall,color=if(sel)Color.White else MintifiColors.TextSecondary)}}}
            Button(onClick={onSave(task.copy(title=title,detail=detail,priority=priority,status=status,deadline=deadline,owner=owner,category=category,projectGroup=project))},modifier=Modifier.fillMaxWidth(),enabled=title.isNotBlank(),colors=ButtonDefaults.buttonColors(containerColor=MintifiColors.Accent),shape=RoundedCornerShape(10.dp)){Icon(Icons.Default.Save,null,tint=Color.White);Spacer(Modifier.width(8.dp));Text("Save Changes",color=Color.White,fontWeight=FontWeight.SemiBold)}
        }
    }
}
...
fun CopilotScreen(viewModel: MainViewModel) {
    ...
    val quickQ=listOf("P0 tasks?","DDR status?","Collections update?","EOD summary draft","AI projects?","Kya karna hai aaj?")
    Column(Modifier.fillMaxSize()){
        Column(Modifier.background(MintifiColors.Surface).padding(horizontal=16.dp,vertical=12.dp)){
            Row(verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(10.dp)){
                ...
                Column{Text("AI Copilot",style=MaterialTheme.typography.titleMedium,color=MintifiColors.TextPrimary);Text("Hindi/English → English responses  •  Brief first",style=MaterialTheme.typography.bodySmall,color=MintifiColors.TextSecondary)}
            }
            Spacer(Modifier.height(8.dp))
        ...
        LazyColumn(state=listState,modifier=Modifier.weight(1f),contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
            ...
            if(isLoading)item{
                val inf=rememberInfiniteTransition(label="t");val a by inf.animateFloat(0.3f,1f,infiniteRepeatable(tween(500),RepeatMode.Reverse),label="a")
        ...
        Row(Modifier.background(MintifiColors.Surface).padding(horizontal=12.dp,vertical=10.dp),horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.Bottom){
            OutlinedTextField(value=inputText,onValueChange={inputText=it},placeholder={Text("Hindi ya English mein puchho...",color=MintifiColors.TextMuted)},modifier=Modifier.weight(1f),colors=OutlinedTextFieldDefaults.colors(focusedBorderColor=MintifiColors.Accent,unfocusedBorderColor=MintifiColors.Border,focusedTextColor=MintifiColors.TextPrimary,unfocusedTextColor=MintifiColors.TextPrimary,cursorColor=MintifiColors.Accent,focusedContainerColor=MintifiColors.Surface2,unfocusedContainerColor=MintifiColors.Surface2),shape=RoundedCornerShape(10.dp),maxLines=4)
...
private fun AddReminderSheet(existing: Reminder?,onSave: (Reminder)->Unit,onDismiss: ()->Unit){
    ...
    var category by remember{mutableStateOf(existing?.category?:ReminderCategory.GENERAL)}
    var repeat by remember{mutableStateOf(existing?.repeatType?:RepeatType.WEEKDAYS)}
    ModalBottomSheet(onDismissRequest=onDismiss,containerColor=MintifiColors.Surface,dragHandle={Box(Modifier.padding(8.dp).width(40.dp).height(4.dp).background(MintifiColors.Border,RoundedCornerShape(2.dp)))}){
...
fun TodoScreen(viewModel: MainViewModel) {
    ...
    todoToDelete?.let{todo->
        AlertDialog(onDismissRequest={todoToDelete=null},title={Text("Delete?")},text={Text("\""+todo.title+"\" will be deleted.")},
    ...
    Box(Modifier.fillMaxSize()){
        Column(Modifier.fillMaxSize()){
            ...
            LazyColumn(contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(6.dp)){
                ...
                items(pending,key={it.id}){todo->TodoCard(todo=todo,onCheck={viewModel.toggleTodo(todo.id,true)},onDelete={todoToDelete=todo})}
                if(completed.isNotEmpty()){item{Row(Modifier.fillMaxWidth().padding(top=8.dp),horizontalArrangement=Arrangement.SpaceBetween,verticalAlignment=Alignment.CenterVertically){Text("Completed ("+completed.size+")",style=MaterialTheme.typography.labelMedium,color=MintifiColors.TextMuted);TextButton(onClick={viewModel.clearCompleted(selectedList)},colors=ButtonDefaults.textButtonColors(contentColor=MintifiColors.DangerRed)){Text("Clear All")}}};items(completed,key={it.id}){todo->TodoCard(todo=todo,onCheck={viewModel.toggleTodo(todo.id,false)},onDelete={todoToDelete=todo},done=true)}}
                item{Spacer(Modifier.height(80.dp))}
            }
        }
...
private fun TodoCard(todo: TodoItem,onCheck: ()->Unit,onDelete: ()->Unit,done: Boolean=false){
    Card(colors=CardDefaults.cardColors(containerColor=if(done)MintifiColors.Surface2 else MintifiColors.Surface),shape=RoundedCornerShape(10.dp),border=BorderStroke(0.5.dp,MintifiColors.Border),modifier=Modifier.fillMaxWidth()){
        Row(Modifier.padding(12.dp),horizontalArrangement=Arrangement.spacedBy(10.dp),verticalAlignment=Alignment.CenterVertically){
            ...
            IconButton(onClick=onDelete,modifier=Modifier.size(30.dp)){Icon(Icons.Default.Delete,"Delete",tint=MintifiColors.DangerRed.copy(alpha=if(done)0.4f else 0.7f),modifier=Modifier.size(16.dp))}
        }
    }
}
...
fun SettingsScreen() {
    ...
    Column(Modifier.fillMaxSize().background(MintifiColors.Background).verticalScroll(rememberScrollState()).padding(20.dp),verticalArrangement=Arrangement.spacedBy(20.dp)){
        ...
        SSection("What's Active in v3"){
            SInfoRow("Recording Storage","App storage + Downloads/CEO_OS_Recordings")
            SInfoRow("Audio Playback","Play any past session from Home screen")
            SInfoRow("Regenerate Tasks","Re-run AI on any saved recording")
            SInfoRow("Speaker Context","CEO button = P0/P1 priority weighted higher")
            SInfoRow("Offline Mode","Queues recordings, processes when reconnected")
            SInfoRow("Auto To-Do","P0+P1 tasks auto-added to Today's list")
            SInfoRow("Copilot Speed","Groq first (fastest), brief first responses")
            SInfoRow("Hinglish Support","All inputs understood, English output always")
        }
...
@Composable private fun SInfoRow(label: String,value: String){Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){Text(label,style=MaterialTheme.typography.bodySmall,color=MintifiColors.TextSecondary);Text(value,style=MaterialTheme.typography.bodySmall,color=MintifiColors.TextPrimary,textAlign=androidx.compose.ui.text.style.TextAlign.End,modifier=Modifier.weight(1f).padding(start=8.dp))}}
